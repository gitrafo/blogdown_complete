...
# Simple exploratory analysis (static vs. interactive)
...
## Top n movies by country - the "static" approach first
...
## Top n movies by country - interactive (Shiny) approach

Simple user interface in Shiny allows interactive data visualization much simpler, here just the screenshot with code example (R-server neccessary in order to run Shiny apps).

![](https://imgur.com/r68yUFs)

The code itself is quite self-explanatory. 

```{r, eval=FALSE, include=TRUE}
# build UI
inputPanel(selectInput(inputId = "country", label = "country", choices = df %>% 
              filter(num_voted_users > 10000) %>% 
              distinct(country) %>% 
              .$country %>% 
              sort),
           
           sliderInput(inputId = "slider", label = "top n", min = 0, max = 20, value = 10, step = 1)
           )

# create reactive variable df_shiny
df_shiny <- reactive({
  df %>% 
  filter(num_voted_users > 10000) %>% 
...
# render plot if df_shiny has changed
renderPlot({
  df_shiny() %>% 
    ggplot(aes(reorder(movie_title, imdb_score), imdb_score, fill = country)) +
    geom_bar(stat = "identity") +
    scale_y_continuous(breaks = seq(0,10,1)) +
    coord_flip() +
    labs(title = paste("Top",input$slider,"movies according to IMDB scores"),
         y = "IMDB score",
         x = "Movie title") +
    theme(plot.title = element_text(face = "bold",
                                    size = 16,
